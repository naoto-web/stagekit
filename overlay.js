...
(function () {
  ...
  /* ══════════ 選手リスペクト演出 "thanks"（8/23 Naoto案） ══════════
     ...
     ✅8/25 本番の抽選に投入済み（THX_IN_ROTATION=true）＝全員共通の演出として、
       2個持ちは各1/3・1個持ちは各1/2で出る（Naoto指定の配分）
     ...
     ══ 発表順（8/25夜 Naoto指示＝1着→2着→3着へ変更・検証中） ══
       ・競輪の実況・場内発表と同じ順を優先（視聴者の耳に馴染んだ順）。
         各着の扱い（色・回転・煽り）は**着順に紐づけたまま**＝1着は先頭でも金・ロングリーチ
       ・⚠️経緯＝8/23には逆の「3着→2着→1着」（表彰式型・弱→強のステップアップ）を
         Naoto承認で採用していた。盛り上がり重視なら321・実況準拠なら123＝見て決める。
         切り戻し＝THX_STEPSの3行を丸ごと並べ替えるだけ（tier等の値は行に付けたまま動かす）
  ...
  /* 全色の抽選に "thanks" を1つ足す＝誰が当てても出る全員共通の演出。
     ✅8/25 Naoto承認で本番投入＝テスト環境で実レース確認済み。
     確率＝Naoto指定どおり：専用演出2個持ち（青・橙）は各1/3、1個持ち（桃・緑・黄・赤）は各1/2。
     リストに1個足すだけでこの配分になる（一様抽選のため）。切り戻し＝falseに戻して再publish */
